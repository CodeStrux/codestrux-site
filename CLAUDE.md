...
- **Tokens** are OKLCH (emerald hue ~152, amber/cyan as sparing syntax pops); `--radius` ~10px, soft "window" pane chrome with brand-tinted traffic-light dots (deliberately diverges from the Stratos zero-radius/hard-shadow language). The **`codestrux-design-language` skill** (`~/work/self/claude-code-skills/`) codifies the full fingerprint.
